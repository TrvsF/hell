...
// #endif
var MemoryEditor = /** @class */ (function () {
    ...
    ;
    ;
    MemoryEditor.prototype.GotoAddrAndHighlight = function (addr_min, addr_max) {
        this.GotoAddr = addr_min;
        this.HighlightMin = addr_min;
        this.HighlightMax = addr_max;
    };
    MemoryEditor.prototype.CalcSizes = function (s, mem_size, base_display_addr) {
        var style = ImGui.GetStyle();
        s.AddrDigitsCount = this.OptAddrDigitsCount;
        if (s.AddrDigitsCount === 0)
            for (var /*size_t*/ n = base_display_addr + mem_size - 1; n > 0; n >>= 4)
                s.AddrDigitsCount++;
    ...
    MemoryEditor.prototype.DrawWindow = function (title, mem_data, mem_size, base_display_addr) {
        var _this = this;
        if (mem_size === void 0) { mem_size = mem_data.byteLength; }
        if (base_display_addr === void 0) { base_display_addr = 0x0000; }
        var s = new MemoryEditor.Sizes();
        ...
        this.Open = true;
        if (ImGui.Begin(title, function (_) {
            if (_ === void 0) { _ = _this.Open; }
            return _this.Open = _;
    ...
    MemoryEditor.prototype.DrawContents = function (mem_data, mem_size, base_display_addr) {
        var _this = this;
        if (mem_size === void 0) { mem_size = mem_data.byteLength; }
        if (base_display_addr === void 0) { base_display_addr = 0x0000; }
        if (this.Cols < 1)
        ...
        var line_total_count = 0 | /*(int)*/ ((mem_size + this.Cols - 1) / this.Cols);
        // ImGuiListClipper clipper;
        var clipper = new ImGui.ListClipper();
        clipper.Begin(line_total_count, s.LineHeight);
        clipper.Step();
        var visible_start_addr = clipper.DisplayStart * this.Cols;
        var visible_end_addr = clipper.DisplayEnd * this.Cols;
        var data_next = false;
        ...
        var preview_data_type_size = this.OptShowDataPreview ? this.DataTypeGetSize(this.PreviewDataType) : 0;
        var data_editing_addr_backup = this.DataEditingAddr;
        ...
        if (data_editing_addr_next !== -1 && (data_editing_addr_next / this.Cols) !== (data_editing_addr_backup / this.Cols)) {
            ...
            var scroll_offset = ( /*(int)*/(data_editing_addr_next / this.Cols) - /*(int)*/ (data_editing_addr_backup / this.Cols));
            var scroll_desired = (scroll_offset < 0 && data_editing_addr_next < visible_start_addr + this.Cols * 2) || (scroll_offset > 0 && data_editing_addr_next > visible_end_addr - this.Cols * 2);
            if (scroll_desired)
        ...
        var color_text = ImGui.GetColorU32(ImGui.Col.Text);
        var color_disabled = this.OptGreyOutZeroes ? ImGui.GetColorU32(ImGui.Col.TextDisabled) : color_text;
        // const char* format_address = this.OptUpperCaseHex ? "%0*" _PRISizeT "X: " : "%0*" _PRISizeT "x: ";
        var format_address = function (n, a) {
            var s = a.toString(16).padStart(n, "0");
            if (_this.OptUpperCaseHex) {
                s = s.toUpperCase();
            }
            return s;
        };
        // const char* format_data = this.OptUpperCaseHex ? "%0*" _PRISizeT "X" : "%0*" _PRISizeT "x";
        var format_data = function (n, a) {
            var s = a.toString(16).padStart(n, "0");
            if (_this.OptUpperCaseHex) {
                s = s.toUpperCase();
            }
        ...
        var format_byte_space = function (b) {
            return "".concat(format_byte(b), " ");
        };
        for (var /*int*/ line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible lines
         {
            var addr = (line_i * this.Cols);
            // ImGui.Text(format_address, s.AddrDigitsCount, base_display_addr + addr);
            ImGui.Text(format_address(s.AddrDigitsCount, base_display_addr + addr));
            // Draw Hexadecimal
            for (var /*int*/ n = 0; n < this.Cols && addr < mem_size; n++, addr++) {
                var byte_pos_x = s.PosHexStart + s.HexCellWidth * n;
                if (this.OptMidColsCount > 0)
                    byte_pos_x += /*(float)*/ (n / this.OptMidColsCount) * s.SpacingBetweenMidCols;
                ImGui.SameLine(byte_pos_x);
                // Draw highlight
                var is_highlight_from_user_range = (addr >= this.HighlightMin && addr < this.HighlightMax);
                var is_highlight_from_user_func = (this.HighlightFn !== null && this.HighlightFn(mem_data, addr));
                var is_highlight_from_preview = (addr >= this.DataPreviewAddr && addr < this.DataPreviewAddr + preview_data_type_size);
                if (is_highlight_from_user_range || is_highlight_from_user_func || is_highlight_from_preview) {
                    var pos = ImGui.GetCursorScreenPos();
                    var highlight_width = s.GlyphWidth * 2;
                    var is_next_byte_highlighted = (addr + 1 < mem_size) && ((this.HighlightMax !== -1 && addr + 1 < this.HighlightMax) || (this.HighlightFn !== null && this.HighlightFn(mem_data, addr + 1)));
                ...
                if (this.DataEditingAddr === addr) {
                    ...
                    var UserData = /** @class */ (function () {
                        ...
                        UserData.Callback = function (data) {
                    ...
                    var flags = ImGui.InputTextFlags.CharsHexadecimal | ImGui.InputTextFlags.EnterReturnsTrue | ImGui.InputTextFlags.AutoSelectAll | ImGui.InputTextFlags.NoHorizontalScroll | ImGui.InputTextFlags.AlwaysOverwrite | ImGui.InputTextFlags.CallbackAlways;
                    ...
                    var data_input_value /*unsigned int*/ = 0;
                    ...
                    var b /*ImU8*/ = this.ReadFn ? this.ReadFn(mem_data, addr) : new Uint8Array(mem_data)[addr];
                    if (this.OptShowHexII) {
                        if ((b >= 32 && b < 128))
                            // ImGui.Text(".%c ", b);
                            ImGui.Text(".".concat(String.fromCharCode(b), " "));
            ...
            if (this.OptShowAscii) {
                ...
                for (var /*int*/ n = 0; n < this.Cols && addr < mem_size; n++, addr++) {
                    ...
                    var c = this.ReadFn ? this.ReadFn(mem_data, addr) : new Uint8Array(mem_data)[addr];
                    // char display_c = (c < 32 || c >= 128) ? '.' : c;
                    var display_c = (c < 32 || c >= 128) ? "." : String.fromCharCode(c);
        ...
        var lock_show_data_preview = this.OptShowDataPreview;
        if (this.OptShowOptions) {
            ImGui.Separator();
    ...
    MemoryEditor.prototype.DrawOptionsLine = function (s, mem_data, mem_size, base_display_addr) {
        var _this = this;
        // IM_UNUSED(mem_data);
        var style = ImGui.GetStyle();
        // const char* format_range = OptUpperCaseHex ? "Range %0*" _PRISizeT "X..%0*" _PRISizeT "X" : "Range %0*" _PRISizeT "x..%0*" _PRISizeT "x";
        var format_range = function (n_min, a_min, n_max, a_max) {
            var s_min = a_min.toString(16).padStart(n_min, "0");
            var s_max = a_max.toString(16).padStart(n_max, "0");
            if (_this.OptUpperCaseHex) {
                s_min = s_min.toUpperCase();
                s_max = s_max.toUpperCase();
            }
            return "Range ".concat(s_min, "..").concat(s_max);
        };
        // Options menu
        ...
        if (ImGui.BeginPopup("context")) {
            ...
            if (ImGui.DragInt("##cols", function (_) {
                if (_ === void 0) { _ = _this.Cols; }
                return _this.Cols = _;
            }, 0.2, 4, 32, "%d cols")) {
            ...
            ImGui.PopItemWidth();
            ImGui.Checkbox("Show Data Preview", function (_) {
                if (_ === void 0) { _ = _this.OptShowDataPreview; }
                return _this.OptShowDataPreview = _;
            });
            ImGui.Checkbox("Show HexII", function (_) {
                if (_ === void 0) { _ = _this.OptShowHexII; }
                return _this.OptShowHexII = _;
            });
            if (ImGui.Checkbox("Show Ascii", function (_) {
                if (_ === void 0) { _ = _this.OptShowAscii; }
                return _this.OptShowAscii = _;
            })) {
                this.ContentsWidthChanged = true;
            }
            ImGui.Checkbox("Grey out zeroes", function (_) {
                if (_ === void 0) { _ = _this.OptGreyOutZeroes; }
                return _this.OptGreyOutZeroes = _;
            });
            ImGui.Checkbox("Uppercase Hex", function (_) {
                if (_ === void 0) { _ = _this.OptUpperCaseHex; }
                return _this.OptUpperCaseHex = _;
            });
            ImGui.EndPopup();
        }
        ImGui.SameLine();
    ...
    MemoryEditor.prototype.DrawPreviewLine = function (s, mem_data, mem_size, base_display_addr) {
        var _this = this;
        ...
        if (ImGui.BeginCombo("##combo_type", this.DataTypeGetDesc(this.PreviewDataType), ImGui.ComboFlags.HeightLargest)) {
            for (var /*int*/ n = 0; n < ImGui.DataType.COUNT; n++)
        ...
        ImGui.Combo("##combo_endianess", function (_) {
            if (_ === void 0) { _ = _this.PreviewEndianess; }
            return _this.PreviewEndianess = _;
        }, "LE\0BE\0\0");
        ImGui.PopItemWidth();
        // char buf[128] = "";
        var buf = new ImGui.StringBuffer(128);
        var x = s.GlyphWidth * 6.0;
        var has_value = this.DataPreviewAddr !== -1;
        if (has_value)
    ...
    MemoryEditor.prototype.DataTypeGetDesc = function (data_type) {
        var descs = ["Int8", "Uint8", "Int16", "Uint16", "Int32", "Uint32", "Int64", "Uint64", "Float", "Double"];
        ImGui.ASSERT(data_type >= 0 && data_type < ImGui.DataType.COUNT);
        return descs[data_type];
    };
    MemoryEditor.prototype.DataTypeGetSize = function (data_type) {
        var sizes = [1, 1, 2, 2, 4, 4, 8, 8, /*sizeof(float)*/ 4, /*sizeof(double)*/ 8];
        ImGui.ASSERT(data_type >= 0 && data_type < ImGui.DataType.COUNT);
        return sizes[data_type];
    };
    MemoryEditor.prototype.DataFormatGetDesc = function (data_format) {
        var descs = ["Bin", "Dec", "Hex"];
        ImGui.ASSERT(data_format >= 0 && data_format < MemoryEditor.DataFormat.COUNT);
        return descs[data_format];
    };
    MemoryEditor.prototype.IsBigEndian = function () {
        // uint16_t x = 1;
        var x = new Uint16Array([1]);
        // char c[2];
        // memcpy(c, &x, 2);
        var c = new Int8Array(x.buffer);
        return c[0] !== 0;
    };
    ...
    // }
    // [Internal]
    MemoryEditor.prototype.DrawPreviewData = function (addr, mem_data, mem_size, data_type, data_format, out_buf, out_buf_size) {
        // uint8_t buf[8];
        var buf = new Uint8Array(8);
        var elem_size = this.DataTypeGetSize(data_type);
        var size = addr + elem_size > mem_size ? mem_size - addr : elem_size;
        if (this.ReadFn)
            for (var /*int*/ i = 0, n = /*(int)*/ size; i < n; ++i)
            ...
            case ImGui.DataType.U8:
                {
                    ...
                    var uint8 = new Uint8Array(buf.buffer);
                    if (data_format === MemoryEditor.DataFormat.Dec) {
                        out_buf.buffer = uint8[0].toString();
                        return;
                    }
                    if (data_format === MemoryEditor.DataFormat.Hex) {
                        out_buf.buffer = "0x".concat(uint8[0].toString(16).padStart(2, "0"));
                        return;
                    }
                    break;
                }
            case ImGui.DataType.S16:
                {
                    ...
                    var int16 = new Int16Array(buf.buffer);
                    if (data_format === MemoryEditor.DataFormat.Dec) {
                        out_buf.buffer = int16[0].toString();
                        return;
                    }
                    if (data_format === MemoryEditor.DataFormat.Hex) {
                        out_buf.buffer = "0x".concat(int16[0].toString(16).padStart(4, "0"));
                        return;
                    }
                    break;
                }
            case ImGui.DataType.U16:
                {
                    ...
                    var uint16 = new Uint16Array(buf.buffer);
                    if (data_format === MemoryEditor.DataFormat.Dec) {
                        out_buf.buffer = uint16[0].toString();
                        return;
                    }
                    if (data_format === MemoryEditor.DataFormat.Hex) {
                        out_buf.buffer = "0x".concat(uint16[0].toString(16).padStart(4, "0"));
                        return;
                    }
                    break;
                }
            case ImGui.DataType.S32:
                {
                    ...
                    if (data_format === MemoryEditor.DataFormat.Hex) {
                        out_buf.buffer = "0x".concat(int32[0].toString(16).padStart(8, "0"));
                        return;
                    }
                    break;
                }
            case ImGui.DataType.U32:
                {
                    ...
                    var uint32 = new Uint32Array(buf.buffer);
                    if (data_format === MemoryEditor.DataFormat.Dec) {
                        out_buf.buffer = uint32[0].toString();
                        return;
                    }
                    if (data_format === MemoryEditor.DataFormat.Hex) {
                        out_buf.buffer = "0x".concat(uint32[0].toString(16).padStart(8, "0"));
                        return;
                    }
                    break;
                }
            case ImGui.DataType.S64:
                {
                    ...
                    var int64 = new BigInt64Array(buf.buffer);
                    if (data_format === MemoryEditor.DataFormat.Dec) {
                        out_buf.buffer = int64[0].toString();
                        return;
                    }
                    if (data_format === MemoryEditor.DataFormat.Hex) {
                        out_buf.buffer = "0x".concat(int64[0].toString(16).padStart(16, "0"));
                        return;
                    }
                    break;
                }
            case ImGui.DataType.U64:
                {
                    ...
                    var uint64 = new BigUint64Array(buf.buffer);
                    if (data_format === MemoryEditor.DataFormat.Dec) {
                        out_buf.buffer = uint64[0].toString();
                        return;
                    }
                    if (data_format === MemoryEditor.DataFormat.Hex) {
                        out_buf.buffer = "0x".concat(uint64[0].toString(16).padStart(16, "0"));
                        return;
                    }
                    break;
                }
            case ImGui.DataType.Float:
                {
                    ...
                    if (data_format === MemoryEditor.DataFormat.Hex) {
                        out_buf.buffer = "0x".concat(float32[0].toString(16));
                        return;
                    }
                    break;
                }
            case ImGui.DataType.Double:
                {
                    ...
                    var float64 = new Float64Array(buf.buffer);
                    if (data_format === MemoryEditor.DataFormat.Dec) {
                        out_buf.buffer = float64[0].toString();
                        return;
                    }
                    if (data_format === MemoryEditor.DataFormat.Hex) {
                        out_buf.buffer = "0x".concat(float64[0].toString(16));
                        return;
                    }
                    break;
                }
...
})(MemoryEditor || (exports.MemoryEditor = MemoryEditor = {}));
// #undef _PRISizeT
// #undef ImSnprintf
// #ifdef _MSC_VER
// #pragma warning (pop)
